...
import {
  ...
  queryManagerReducer,
  init,
} from '../reducers/queryManagerReducer';
...
export const useQueryManager = (): UseQueryManagerResult => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      localStorage.getItem('sqlQueryHistory') !== JSON.stringify([])
    ) {
      try {
  ...
  useEffect(() => {
    ...
    if (
      ...
      localStorage.getItem('sqlSavedQueries') !== JSON.stringify([])
    ) {
      try {
  ...
  const runQueryLogic = useCallback(
    (query: string, dataToDisplay: QueryData | QueryResultState | null) => {
      dispatch({ type: 'SET_LOADING', payload: true });
      dispatch({
        type: 'SET_QUERY_RESULT',
        payload: { message: 'Loading results...', status: 'loading' },
      });
      ...
      setTimeout(() => {
        ...
        if (dataToDisplay && 'headers' in dataToDisplay) {
          ...
          finalResult = {
            message: 'No data or specific result for this query.',
            status: 'success',
          };
        }
  ...
  const handleRunQuery = useCallback(() => {
    ...
    if (query === '') {
      dispatch({
        type: 'SET_QUERY_RESULT',
        payload: { message: 'Please enter an SQL query.', status: 'error' },
      });
      return;
    }
  ...
  const handleSaveQuery = useCallback((name: string, queryToSave: string) => {
    ...
    const savedLocalQueries = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_SAVED_QUERIES_KEY) || '[]'
    );
    ...
    return {
      data: newSavedQuery,
      success: true,
      message: `Query "${trimmedName}" saved successfully.`,
    };
  }, []);
