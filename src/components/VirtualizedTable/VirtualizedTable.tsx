    ...
    return (
        <div className="table-responsive-wrapper" ref={tableScrollWrapperRef}>
            ...
            {rows.length > 0 ? (
                ...
                <div className="no-filter-results-fixed-size" style={{ height: 300, width: totalContentWidth, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
const innerElementType = React.forwardRef<HTMLDivElement, React.HTMLProps<HTMLDivElement>>(
    ({ style, ...rest }, ref) => {
        ...
        const totalWidth = itemDataFromProps?.columnWidths.reduce((sum: number, width: number) => sum + width, 0) || 0;
