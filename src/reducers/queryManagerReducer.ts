import { QueryManagerState, QueryManagerAction } from '../types';
import { HISTORY_MAX_SIZE, LOCAL_STORAGE_KEY, LOCAL_STORAGE_SAVED_QUERIES_KEY } from '../constants';
...
export const queryManagerReducer = (
  ...
  switch (action.type) {
    ...
    case 'ADD_OR_UPDATE_SAVED_QUERY':
      const existingQueryIndex = state.savedQueries.findIndex(
        (q) => q.name.toLowerCase() === action.payload.name.toLowerCase()
      );
